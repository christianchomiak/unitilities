...
public static class MathHelper
{
    ...
    {
        return MathHelper.GenerateMirrorPair(_origin, _direction, Random.Range(0f, _maxDistanceInLine), Random.Range(0f, _maxDistanceFromLine), Random.Range(0f, 1f) <= 0.5f, Random.Range(0f, 1f) <= 0.5f);
    }
    ...
    public static Vector2[] GenerateMirrorPair(Vector2 _origin, Vector2 _direction, float _distanceInLine, float _distanceFromLine, bool _goAgainstDirection, bool _swapHemispheres)
    {
        ...
        float distanceInLine = _distanceInLine; // Random.Range(0f, maxDistance);

        //Length of the rejection vector
        float distanceFromLine = _distanceFromLine; // Random.Range(0f, maxDistance);

        Vector2 projection = _direction;

        //The projection can go with or against the _direction
        if (_goAgainstDirection) // Random.Range(0f, 1f) <= 0.5f)
        {
            projection *= -1;
        }
        ...
        if (_goAgainstDirection) // !_swapHemispheres && // Random.Range(0f, 1f) <= 0.5f)
        {
            rejection *= -1f;
        }
    ...
    {
        return MathHelper.GenerateMirrorPairRadially(_origin, _direction, Random.Range(1f, _maxRadius), Random.Range(0f, 180f));
    }
    ...
    public static Vector2[] GenerateMirrorPairRadially(Vector2 _origin, Vector2 _direction, float _radius, float _deltaAngle)
    {
        ...
        float distance = _radius; //Random.Range(0f, float.MaxValue);
        ...
        float deltaAngle = _deltaAngle; // Random.Range(0f, 180f);
